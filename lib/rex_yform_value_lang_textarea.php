...
class rex_yform_value_lang_textarea extends rex_yform_value_abstract
{
    public function enterObject()
    {
        ...
        if (isset($_POST['FORM']) && isset($_POST['FORM'][$this->params['form_name']]['send'])) {
            ...
            if (rex::isBackend()) {
                error_log('YForm Lang Textarea Debug POST Processing: ' . json_encode([
                    'form_name' => $formName,
                    'field_id' => $fieldId,
            ...
            if (isset($_POST['FORM'][$formName][$fieldId]) && is_array($_POST['FORM'][$formName][$fieldId])) {
                ...
                error_log('YForm Lang Textarea saved: ' . $this->getName() . ' = ' . $this->getValue());
            }
        }
    }
    ...
    protected function getTemplateParams(): array
    {
        ...
        if (!empty($attributes)) {
            // JSON-Attribute parsen oder String-Attribute
            if (strpos($attributes, '{') === 0) {
                $parsedAttributes = json_decode($attributes, true) ?: [];
            } else {
                // Einfache Attribute parsen (rows="5", class="cke5")
                if (preg_match_all('/([^=\s]+)="([^"]*)"|([^=\s]+)=([^\s]+)/', $attributes, $matches, PREG_SET_ORDER)) {
                    foreach ($matches as $match) {
                        $key = !empty($match[1]) ? $match[1] : $match[3];
                        $value = !empty($match[1]) ? $match[2] : $match[4];
        ...
        return [
            ...
            'use_editor' => isset($parsedAttributes['class']) && strpos($parsedAttributes['class'], 'cke5') !== false
        ];
    }
